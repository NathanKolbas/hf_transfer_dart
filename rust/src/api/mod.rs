...
use flutter_rust_bridge::for_generated::anyhow::{anyhow, bail, Result};
...
pub fn download(
  ...
  max_files: usize,
  chunk_size: usize,
  parallel_failures: Option<usize>,
  max_retries: Option<usize>,
  headers: Option<HashMap<String, String>>,
  // frb doesn't yet support passing function wrapped by an Option. Handled on the dart side.
  callback: impl Fn(usize) -> DartFnFuture<()> + Send + Sync + 'static,
) -> Result<()> {
  // Setup defaults
  let parallel_failures = parallel_failures.unwrap_or(0);
  let max_retries = max_retries.unwrap_or(0);

  if parallel_failures > max_files {
    bail!("Error parallel_failures cannot be > max_files".to_string());
  }
  if (parallel_failures == 0) != (max_retries == 0) {
    bail!(
      "For retry mechanism you need to set both `parallel_failures` and `max_retries`".to_string(),
    );
  }
    ...
    .map_err(|err| {
      ...
      if path.exists() {
        match remove_file(filename) {
          Ok(_) => err,
          Err(err) => anyhow!("Error while removing corrupted file: {err}")
        }
      } else {
        err
      }
...
pub fn multipart_upload(
  ...
  max_retries: Option<usize>,
  // frb doesn't yet support passing function wrapped by an Option. Handled on the dart side.
  callback: impl Fn(u64) -> DartFnFuture<()> + Send + Sync + 'static,
  ...
  if (parallel_failures == 0) != (max_retries == 0) {
    bail!("For retry mechanism you need to set both `parallel_failures` and `max_retries`".to_string());
  }
  ...
  if let Some(input_headers) = input_headers {
    ...
    for (k, v) in input_headers {
      ...
      let value: HeaderValue = AsRef::<str>::as_ref(&v)
        .try_into()
        .map_err(|err| anyhow!("Invalid header value: {err}"))?;
    ...
    handles.push(tokio::spawn(async move {
      ...
      if parallel_failures > 0 {
        while let Err(dlerr) = chunk {
          if i >= max_retries {
            return Err(anyhow!("Failed after too many retries ({max_retries}): {dlerr}"));
          }
...
async fn upload_async(
  ...
  for (part_number, part_url) in parts_urls.iter().enumerate() {
    ...
    handles.push(tokio::spawn(async move {
      let permit = semaphore
        ...
        .acquire_owned()
        .await
        .map_err(|err| anyhow!("Error acquiring semaphore: {err}"))?;
      let mut chunk = upload_chunk(&client, &url, &path, start, chunk_size).await;
      let mut i = 0;
      if parallel_failures > 0 {
        while let Err(ul_err) = chunk {
          if i >= max_retries {
            return Err(anyhow!("Failed after too many retries ({max_retries}): {ul_err}"));
          }

          let parallel_failure_permit = parallel_failures_semaphore.clone().try_acquire_owned().map_err(|err| {
            anyhow!("Failed too many failures in parallel ({parallel_failures}): {ul_err} ({err})")
          })?;
      ...
      chunk.map_err(|e|{
        match e {
          Error::Io(io) => anyhow!("Io error {io}"),
          Error::Request(req) => anyhow!("Error while sending chunk {req}"),
          Error::ToStrError(req) => anyhow!("Response header contains non ASCII chars: {req}"),
        }
  ...
  while let Some(result) = handles.next().await {
    match result {
      ...
      Err(err) => {
        return Err(anyhow!("Error occurred while uploading: {err}"));
      }
    }
  }
...
pub fn version() -> String { env!("CARGO_PKG_VERSION").clone().to_string() }
